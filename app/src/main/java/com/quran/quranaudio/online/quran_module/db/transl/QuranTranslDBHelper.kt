 ...
 */
package com.quran.quranaudio.online.quran_module.db.transl
...
import com.quran.quranaudio.online.quran_module.db.transl.QuranTranslContract.QuranTranslEntry.*
import com.quran.quranaudio.online.quran_module.db.transl.QuranTranslInfoContract.QuranTranslInfoEntry
import com.quran.quranaudio.online.quran_module.utils.Log
import com.quran.quranaudio.online.quran_module.utils.quran.QuranConstants
import java.io.File
...
class QuranTranslDBHelper(private val context: Context) : SQLiteOpenHelper(
    ...
    override fun onCreate(DB: SQLiteDatabase) {
        ...
        DB.beginTransaction()
        try {
            for (bookInfo in com.quran.quranaudio.online.quran_module.utils.reader.TranslUtils.preBuiltTranslBooksInfo()) {
                val prebuiltTranslPath = com.quran.quranaudio.online.quran_module.utils.reader.TranslUtils.getPrebuiltTranslPath(bookInfo.slug)
                val translStrData = com.quran.quranaudio.online.quran_module.utils.univ.StringUtils.readInputStream(
    ...
    private fun migrateFileBasedTranslsToDatabase(context: Context, DB: SQLiteDatabase) {
        val fileUtils = com.quran.quranaudio.online.quran_module.utils.univ.FileUtils.newInstance(context)
        val translDir = File(fileUtils.appFilesDirectory, com.quran.quranaudio.online.quran_module.utils.reader.TranslUtils.DIR_NAME)
        if (!translDir.exists()) return

        try {
            com.quran.quranaudio.online.quran_module.utils.reader.TranslUtils.getTranslInfosAndFilesForMigration(fileUtils, translDir)?.let {
                for (pair in it) {
